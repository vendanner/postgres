 ...
 * 检查参数
 * 初始化
 * MemoryContextInit       // 初始化TOP 内存上下文
 * 设置LC* 环境变量
 * 检查是否以root 启动
 * 检查argv[1] 值
 *      如果等于"--boot"，进入 BootstrapModeMain(argc，argv)   => initDB 命令流程
 *      如果等于"--describe-config",
 *          进入 GucInfoMain(argc，argv)
 *      如果等于"--single"，
 *          进入 PostgresSingleUserMain(argc，argv，null，strdup(get_user_name_or_exit(progname)))
 *      默认 PostmasterMain(argc，argv)
 */
int
main(int argc, char *argv[])
{
    ...
    if (argc > 1 && strcmp(argv[1], "--check") == 0)
        ...
        PostmasterMain(argc, argv);     // src/backend/postmaster.PostmasterMain
