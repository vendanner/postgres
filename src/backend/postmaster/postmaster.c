 ...
 * 默认启动方式 主流程
 *  内存上下文相关，为当前函数的运行，准备内存：进程内私有内存 => AllocSetContextCreate
 *  信号设置 => pqinitmask
 *  设置所有配置参数为0/默认值，此函数没有都去配置文件(后续读取配置文件设置对应值) => InitializeGUCOptions
 *  处理所有的命令行参数，有些命令行参数将用来修改配置参数在内存中的值
 *  读取配置文件并设置参数，参数是 src/backend/utils/init/globals.c 全局变量 => SelectConfigFiles
 *  读取控制文件(数据库状态信息)，并进行一些检查 => LocalProcessControlFile
 *  加载shared_preload_libraries 配置的.so 库 => process_shared_preload_libraries
 *  计算全局变量 MaxBackends => 最大的后台进程数量 => InitializeMaxBackends
 *  创建/分配共享内存 => reset_shared
 *  设置进程最大文件描述符(分配一大块共享内存) => set_max_safe_fds
 *  删除临时文件，清理
 *  启动第一个进程：日志进程 => SysLogger_Start
 *  创建Server 端套接字 => StreamServerPort
 *  初始化 auto vacuum 进程(行清理)，但不启动 => autovac_init
 *  加载配置目录下 hba.conf 文件(设置某个客户端ip 是否能进行网络连接) => load_hba
 *  新建子进程会启动数据库(可能涉及到数据库恢复)，然后子进程自动退出 => StartupDataBase
 *  while(1) 监听客户端请求并相应 => ServerLoop
 */
void
PostmasterMain(int argc, char *argv[])
{
    ...
    // 获取主进程(master) PID，获取当前时间戳，作为启动时间
    InitProcessGlobals();
     ...
     */
    pqinitmask();
    PG_SETMASK(&BlockSig);      // 改变当前进程信号集，设置为BlockSig
    /*
     * SIGHUP 结束时发出
     * SIGINT 终端中断一般为 Ctrl+C 发出
     * SIGQUIT dump	键盘产生的退出
     * SIGTERM 终止	进程终止
     * SIGALRM 终止	时钟中断(闹钟)
     * SIGPIPE 终止	向某个没有读取的管道中写入数据
     * SIGUSR1 自定义信号，可以由用户定义并在需要时使用。用于各个子进程通知、唤醒，需使用kill 触发此信号
     * SIGUSR2 自定义信号，可以由用户定义并在需要时使用。
     * SIGCHLD    	子进程退出或中断，有OS触发，用于pg 启动时(数据库启动 => 加载数据文件是新建子进程)
     * 以上信号都不在 BlockSig 中，表示当前不处理以上信号
     */
    ...
    /*
     * Options setup
     * 设置所有配置参数为0/默认值，此函数没有都去配置文件(后续读取配置文件设置对应值) => SelectConfigFiles
     */
     ...
     *
     * 读取配置文件并设置参数，
     * 参数大部分在 src/backend/utils/init/globals.c 全局变量
     */
     ...
     *
     * 加载shared_preload_libraries 配置的.so 库
     */
     ...
     * 计算全局变量 MaxBackends => 最大的后台进程数量
     *       MaxConnections + autovacuum_max_workers + 1 + max_worker_processes + max_wal_senders
     */
     ...
     * 设置进程最大文件描述符(分配一大块共享内存)
     * 最大文件描述符，这个值来自于配置文件，决定pg可以打开的文件数量
     * max_files_per_process 参数，默认1000，
     * 对于oltp 通常是够了，对于olap 1000可能不够，可根据情况适当调大
     */
    set_max_safe_fds();
     ...
     * 调用系统函数pipe(), 创建管道。这个管道会继承所有fork 出来的子进程
     */
     ...
     *
     * 删除 promote、fallback_promote 文件
     */
     ...
     *
     * 网络监听，ListenSocket 数组初始化为 PGINVALID_SOCKET
     */
    ...
    // ListenAddresses 来自配置文件listen_addresses 参数
    // 开启server 端口
    if (ListenAddresses)
    {
        char	   *rawstring;
     ...
     * 初始化 auto vacuum 进程(行清理)，但不启动
     */
    autovac_init();

    /*
     * Load configuration files for client authentication.
     * 加载配置目录下 hba.conf 文件(设置某个ip 是否能进行网络连接)
     */
    if (!load_hba())
    {
        /*
     ...
     * 将字符串"starting" 写入 postmaster.pid(放到最后)
     */
     ...
     * 非常重要，与ServerLoop 共同完成最后的重要任务
     * 实际调用 StartChildProcess(StartupProcess)
     *  新建子进程会启动数据库(可能涉及到数据库恢复)，然后子进程自动退出
     */
 ...
 *
 * 死循环，核心逻辑：select ListenSocket，然后accept 接收请求，BackendStartup 新建子进程处理
 */
static int
ServerLoop(void)
{
    ...
    // 接收客户端连接，处理请求
    // select ListenSocket，然后accept 接收请求，BackendStartup 新建子进程处理
    for (;;)
    {
        fd_set		rmask;
        int			selres;
        ...
        {
            // 设置为 UnBlockSig(放开所有信号)，看pqinitmask
            // 为了查看之前启动数据库的子进程的信号
        ...
        if (selres > 0)
        {
            ...
            for (i = 0; i < MAXLISTEN; i++)
            {
                ...
                {    // 当前套接字是否可读，select 太多套接字了，要判断具体是哪个套接字处于连接中
                    // ListenSocket 每个元素都是一个套接字，大多数情情况ListenSocket=1，
                    // 少部分情况，会为当前机器上每个ip 都建套接字
                    Port	   *port;
                    // 接收一个新的连接
